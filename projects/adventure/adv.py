...
def traverseMap(player, direction=''):
    ...
    # Travel to unexlpored '?' room
    for exit in player.currentRoom.getExits():
        if traversalGraph[currentRoom][exit] == '?':
            # Set the new direction to that exit
            new_direction = exit
            # Append the current exit to the traversal path
            player.travel(exit)
            traversalPath.append(exit)
            # Set the new room to the current room and set the prev room's exit to the new room
    ...
    # Repeat until all the rooms are explored
    traverseMap(player)
